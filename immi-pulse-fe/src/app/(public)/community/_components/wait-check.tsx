      ...
      <div className="px-5 pb-5 pt-5 sm:px-6">
        {!ready ? (
          ...
          <div className="py-6 text-center">
            <p className="text-[15px] font-medium text-ink">{result.headline}</p>
          ...
          <>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-1.5 border-t border-hair pt-3.5 text-[11.5px] text-ink-soft">
              {result.basis === "official" ? (
                <span className="c-mono">
                  Based on official Home Affairs figures
                  {result.official_updated ? ` · ${result.official_updated}` : ""}
                </span>
              ) : (
                <>
                  <span className="c-mono">
                    {result.sample_size.toLocaleString()} community{" "}
                    {result.sample_size === 1 ? "grant" : "grants"}
                  </span>
                  <span className="c-mono">
                    {result.pending.toLocaleString()} still waiting
                  </span>
                  {result.official_p50_days != null && (
                    <span className="c-mono">
                      official median {formatDays(result.official_p50_days)}
                    </span>
                  )}
                </>
              )}
