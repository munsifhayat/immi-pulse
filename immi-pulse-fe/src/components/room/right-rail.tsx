...
/** Search the room. `/` focuses it, the way every platform this borrows from does. */
function RoomSearch() {
  const { search, setSearch } = useRoom();
 ...
 *
 * The official figure alone is an unchecked claim; the room's alone has nothing
 * corroborating it. Neither column is ever rendered without the other, and the
 * room's number carries its composition — timelines collected from public
...
function DualSourceTimes() {
  ...
  const note = rows
    .map((r) => (r.room ?? r.community).provenance_note)
    .filter(Boolean)
  ...
  return (
    <div className="rounded-2xl border border-hair bg-white p-4">
      ...
      <p className="c-mono mt-2.5 text-[9px] leading-[1.7] tracking-[0.05em] text-ink-soft/80">
        ...
        AS-AT DATE
        <br />
        ROOM · MEDIAN OF SHARED TIMELINES LODGED IN THE LAST 12 MONTHS
      </p>
      {note && (
