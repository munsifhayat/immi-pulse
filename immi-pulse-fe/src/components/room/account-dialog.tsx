...
function SignupPanel({ onSwitch }: { onSwitch: () => void }) {
  const { identity, closeAccount } = useRoom();
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-4">
      {/*
        ...
        this room is full of people whose visa status is not safe to attach to
        any of that.
      */}
...
export function AccountDialog() {
  const { accountDialog, closeAccount, openAccount } = useRoom();
