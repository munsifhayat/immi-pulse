...
function ResetPanel({ token }: { token: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    // Straight into the room, already signed in. Someone who has just been
  ...
  if (reset.isSuccess) {
    return (
      <Panel eyebrow="Done" title="You're back in.">
        ...
        <BackToRoom label="Go to the room" />
      </Panel>
    );
  }
...
function BackToRoom({ label = "Back to the room" }: { label?: string }) {
  return (
