...
export function RoomFeed({ type }: { type?: PostType }) {
  const { queue, search } = useRoom();
    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="px-6 py-20 text-center">
          <MessageCircle
            className="mx-auto h-8 w-8 text-ink-soft/35"
            strokeWidth={1.5}
          />
          ...
          <p className="mx-auto mt-1.5 max-w-xs text-[13px] leading-relaxed text-ink-soft">
            {search.trim()
              ...
              : "Ask the room a question, or share where your own application has got to."}
          </p>
        </div>
      ) : (
        <>
