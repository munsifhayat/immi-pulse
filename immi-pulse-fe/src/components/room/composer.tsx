...
import { HelpCircle, Loader2, Lock, Plus, Waves } from "lucide-react";
import {
  useCreateJourney,
  useVisaSubclasses,
 ...
 *
 * Exactly two modes. "Ask the room" is the default because a question is the
...
export function Composer({ onPosted }: { onPosted?: () => void }) {
  const { account, identity, openAccount, canWrite, writeBlock } = useRoom();
  ...
  return (
    <div className="border-b border-hair px-5 pb-4 pt-4">
      ...
      <div className="mb-3.5 inline-flex gap-1 rounded-full border border-hair bg-white p-[3px]">
        {(
          [
            { id: "question", label: "Ask the room", Icon: HelpCircle },
            { id: "timeline", label: "Share your story", Icon: Waves },
          ] as const
        ).map(({ id, label, Icon }) => (
          <button
            key={id}
          ...
          {mode === "question" ? (
            ...
            <div className="pt-1">
              ...
              */}
              <button
                type="button"
                onClick={() => canWrite("post") && setFullBuilderOpen(true)}
            ...
            */}
            {block === "no-account" ? (
              ...
              >
                ...
                {mode === "question" ? "Ask the room" : "Share timeline"}
              </button>
            )}
          </div>
        </div>
      ...
      <ShareJourney
        open={fullBuilderOpen}
        onOpenChange={setFullBuilderOpen}
        identity={identity}
        defaultSubclass={tlSubclass || undefined}
        onCapReached={() => {
